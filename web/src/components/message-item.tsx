        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={cn(
            ...
            isUser
              ...
              : 'glass-card text-white/85 rounded-2xl rounded-tl-sm'
          )}>
            ...
            >
              {isCopied ? (
                ...
                <svg className={cn('w-3 h-3', isUser ? 'text-white/40' : 'text-white/30')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {!isUser && message.trajectory && message.trajectory.length > 0 && (
              <div className="mb-2.5 flex flex-wrap items-center gap-1">
                {message.trajectory.map((step, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-violet-500/10 border border-violet-500/15 text-[9px] font-medium text-violet-300/70"
              ...
              <div className="mt-3 pt-2.5 border-t border-white/[0.06]">
                <div className="flex flex-wrap gap-1.5">
                  {uniqueSources.slice(0, 3).map((ctx, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center space-x-1 px-2 py-1 rounded-md bg-white/[0.05] border border-white/[0.06] text-[10px] text-white/45 max-w-full"
                    >
                      <svg className="w-2.5 h-2.5 flex-shrink-0 text-cyan-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {uniqueSources.length > 3 && (
                    <span className="inline-flex items-center px-2 py-1 rounded-md bg-white/[0.03] text-[10px] text-white/35">
          ...
          {!isUser && message.metrics && (
            <div className="flex flex-wrap items-center gap-2 mt-1.5 ml-1">
              {message.metrics.latency_ms !== undefined && (
                <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-white/[0.04] text-[9px] text-white/35">
                  <svg className="w-2.5 h-2.5 text-cyan-400/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {message.metrics.total_tokens !== undefined && (
                <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-white/[0.04] text-[9px] text-white/35">
                  <svg className="w-2.5 h-2.5 text-violet-400/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {message.metrics.usd_cost !== undefined && (
                <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-white/[0.04] text-[9px] text-white/35">
                  <svg className="w-2.5 h-2.5 text-emerald-400/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {message.metrics.retriever_hits !== undefined && (
                <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-white/[0.04] text-[9px] text-white/35">
                  <svg className="w-2.5 h-2.5 text-amber-400/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className={cn('flex items-center space-x-1.5 mt-1', isUser ? 'justify-end mr-1' : 'justify-start ml-1')}>
            ...
            {isUser && (
              <svg className="w-2.5 h-2.5 text-cyan-400/50" fill="currentColor" viewBox="0 0 24 24">
